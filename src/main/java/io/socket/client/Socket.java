...
public class Socket extends Emitter {
    ...
    private static final Logger logger = Logger.getLogger(io.socket.client.Socket.class.getName());
    ...
    private void subEvents() {
        ...
        final Manager io = io.socket.client.Socket.this.io;
        io.socket.client.Socket.this.subs = new LinkedList<On.Handle>() {{
            add(On.on(io, Manager.EVENT_OPEN, new Listener() {
                @Override
                public void call(Object... args) {
                    io.socket.client.Socket.this.onopen();
                }
            }));
            add(On.on(io, Manager.EVENT_PACKET, new Listener() {
                @Override
                public void call(Object... args) {
                    io.socket.client.Socket.this.onpacket((Packet<?>) args[0]);
                }
            }));
            add(On.on(io, Manager.EVENT_CLOSE, new Listener() {
                @Override
                public void call(Object... args) {
                    io.socket.client.Socket.this.onclose(args.length > 0 ? (String) args[0] : null);
                }
            }));
        }};
    }
     ...
     * Connects the socket.
     */
    public io.socket.client.Socket open() {
        EventThread.exec(new Runnable() {
            @Override
            public void run() {
                if (io.socket.client.Socket.this.connected) return;

                io.socket.client.Socket.this.subEvents();
                io.socket.client.Socket.this.io.open(); // ensure open
                if (Manager.ReadyState.OPEN == io.socket.client.Socket.this.io.readyState) io.socket.client.Socket.this.onopen();
                io.socket.client.Socket.this.emit(EVENT_CONNECTING);
            }
        });
        return this;
    }
     ...
     */
    public io.socket.client.Socket send(final Object... args) {
        EventThread.exec(new Runnable() {
            @Override
            public void run() {
                io.socket.client.Socket.this.emit(EVENT_MESSAGE, args);
            }
        });
        return this;
    }
        ...
        EventThread.exec(new Runnable() {
            ...
            public void run() {
                if (events.containsKey(event)) {
                    io.socket.client.Socket.super.emit(event, args);
                    return;
                }
        ...
        EventThread.exec(new Runnable() {
            ...
            public void run() {
                ...
                if (ack != null) {
                    ...
                    io.socket.client.Socket.this.acks.put(ids, ack);
                    packet.id = ids++;
                }

                if (io.socket.client.Socket.this.connected) {
                    io.socket.client.Socket.this.packet(packet);
                } else {
                    io.socket.client.Socket.this.sendBuffer.add(packet);
                }
            }
        });
        return this;
    ...
    private Ack ack(final int id) {
        final io.socket.client.Socket self = this;
     ...
     * @return a reference to this object.
     */
    public io.socket.client.Socket close() {
        EventThread.exec(new Runnable() {
            @Override
            public void run() {
                if (io.socket.client.Socket.this.connected) {
                    if (logger.isLoggable(Level.FINE)) {
                        logger.fine(String.format("performing disconnect (%s)", io.socket.client.Socket.this.nsp));
                    }
                    io.socket.client.Socket.this.packet(new Packet(Parser.DISCONNECT));
                }

                io.socket.client.Socket.this.destroy();

                if (io.socket.client.Socket.this.connected) {
                    io.socket.client.Socket.this.onclose("io client disconnect");
                }
            }
        });
        return this;
