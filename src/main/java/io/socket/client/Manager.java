...
public class Manager extends Emitter {
    ...
    private static final Logger logger = Logger.getLogger(io.socket.client.Manager.class.getName());
    ...
    public io.socket.client.Manager reconnection(boolean v) {
    ...
    public io.socket.client.Manager reconnectionAttempts(int v) {
    ...
    public io.socket.client.Manager reconnectionDelay(long v) {
    ...
    public io.socket.client.Manager randomizationFactor(double v) {
    ...
    public io.socket.client.Manager reconnectionDelayMax(long v) {
    ...
    public io.socket.client.Manager timeout(long v) {
        this._timeout = v;
        return this;
    }
    ...
    public io.socket.client.Manager open(){
        return open(null);
    }
     ...
     */
    public io.socket.client.Manager open(final OpenCallback fn) {
        EventThread.exec(new Runnable() {
            @Override
            public void run() {
                if (logger.isLoggable(Level.FINE)) {
                    logger.fine(String.format("readyState %s", io.socket.client.Manager.this.readyState));
                }
                if (io.socket.client.Manager.this.readyState == ReadyState.OPEN || io.socket.client.Manager.this.readyState == ReadyState.OPENING) return;

                if (logger.isLoggable(Level.FINE)) {
                    logger.fine(String.format("opening %s", io.socket.client.Manager.this.uri));
                }
                io.socket.client.Manager.this.engine = new Engine(io.socket.client.Manager.this.uri, io.socket.client.Manager.this.opts);
                final io.socket.engineio.client.Socket socket = io.socket.client.Manager.this.engine;
                final io.socket.client.Manager self = io.socket.client.Manager.this;
                io.socket.client.Manager.this.readyState = ReadyState.OPENING;
                io.socket.client.Manager.this.skipReconnect = false;

                // propagate transport event.
                socket.on(Engine.EVENT_TRANSPORT, new Listener() {
                    @Override
                    public void call(Object... args) {
                        self.emit(EVENT_TRANSPORT, args);
                    }
                });
                ...
                if (io.socket.client.Manager.this._timeout >= 0) {
                    final long timeout = io.socket.client.Manager.this._timeout;
                    ...
                    io.socket.client.Manager.this.subs.add(new On.Handle() {
                        @Override
                        public void destroy() {
                            timer.cancel();
                        }
                ...
                io.socket.client.Manager.this.subs.add(openSub);
                io.socket.client.Manager.this.subs.add(errorSub);

                io.socket.client.Manager.this.engine.open();
            }
        });
        return this;
    }
    ...
    private void onopen() {
        ...
        this.subs.add(On.on(socket, Engine.EVENT_DATA, new Listener() {
            ...
            public void call(Object... objects) {
                ...
                if (data instanceof String) {
                    io.socket.client.Manager.this.ondata((String)data);
                } else if (data instanceof byte[]) {
                    io.socket.client.Manager.this.ondata((byte[])data);
                }
            }
        }));
        this.subs.add(On.on(socket, Engine.EVENT_PING, new Listener() {
            @Override
            public void call(Object... objects) {
                io.socket.client.Manager.this.onping();
            }
        }));
        this.subs.add(On.on(socket, Engine.EVENT_PONG, new Listener() {
            @Override
            public void call(Object... objects) {
                io.socket.client.Manager.this.onpong();
            }
        }));
        this.subs.add(On.on(socket, Engine.EVENT_ERROR, new Listener() {
            @Override
            public void call(Object... objects) {
                io.socket.client.Manager.this.onerror((Exception)objects[0]);
            }
        }));
        this.subs.add(On.on(socket, Engine.EVENT_CLOSE, new Listener() {
            @Override
            public void call(Object... objects) {
                io.socket.client.Manager.this.onclose((String)objects[0]);
            }
        }));
        this.decoder.onDecoded(new Parser.Decoder.Callback() {
            @Override
            public void call (Packet packet) {
                io.socket.client.Manager.this.ondecoded(packet);
            }
        });
    }
    ...
    public Socket socket(final String nsp, Options opts) {
        ...
        if (socket == null) {
            ...
            if (_socket != null) {
                ...
                final io.socket.client.Manager self = this;
